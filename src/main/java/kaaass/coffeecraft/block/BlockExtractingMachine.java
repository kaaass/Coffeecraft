...
public class BlockExtractingMachine extends BlockContainer {
	@SideOnly(Side.CLIENT)
	private IIcon[] iconArray;
	
	public BlockExtractingMachine(){
		super(Material.rock);
		setHardness(3.0F);
		setCreativeTab(CoffeecraftMain.creativeTab);
		setBlockName("extractingMachine");
    ...
    public void registerBlockIcons(IIconRegister register){
		...
		iconArray[0] = register.registerIcon(CoffeecraftMain.MODID+":extractingMachine");
	...
	public TileEntity createNewTileEntity(World world, int arg1) {
		return new TileEntityExtractingMachine();
	}
	
	@Override
	public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int par6, float par7, float par8, float par9){
		player.openGui(CoffeecraftMain.instance, Gui.COFFEE_EXTRACT_ID, world, x, y, z);
		return true;
	}
}
